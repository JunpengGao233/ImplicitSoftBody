...
class DiffSim(torch.autograd.Function):
    ...
    def forward(ctx, x0, v0, a, dt, max_iter, robot):
        ...
        with torch.enable_grad():
            dx2 = torch.zeros_like(x0[1]).requires_grad_(True)
            dx1 = torch.zeros_like(x0[0])
        ...
        # for epoch_i in range(max_iter):
            ...
            if torch.norm(dx2-dx0)/dx0.norm() < 1e-5:
                break
        print(f'epoch {epoch_i}: loss {loss.item()}, dx2 {torch.norm(dx2)}')
        # v = dx/dt
        x = x0.detach().clone()
        x[1] = x0[1]+dx2
