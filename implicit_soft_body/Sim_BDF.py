...
import system 
class DiffSimBDF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x1, x0, v1, a, dt, max_iter, robot):
        ctx.robot = robot
        # dx = (dt * v0).requires_grad_(True)
        dx = (1/3 *x1 - 1/3*x0 + 2/3*dt*v1).requires_grad_(True)
        ...
        for epoch_i in range(3000):#range(max_iter):
            ...
            v1 = v1.detach().clone()#.requires_grad_(True)
            def closure():
                optimizer.zero_grad()
                # loss = robot.total_energy(x0, x0+dx, v0, a)
                loss = robot.total_energy_bdf(x0, x1, x1+dx, v1, a)
        ...
        ctx.save_for_backward(x0, x1, x, v1, v, a)
        # print("x ", x)
        # print("a", a)
        # print("l0, ", robot.l0)
    ...
    def backward(ctx, grad_output_x, grad_output_v) -> Any:
        ...
        x0, x1, x, v0, v, a = ctx.saved_tensors
        ...
        return None, None, None, dLda, None, None, None