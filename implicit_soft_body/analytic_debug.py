...
if __name__ == '__main__':
    ...
    x_target = torch.tensor([[0,0 ],[0.9, 0.9]])
    # x_target = torch.ones_like(robot.x) * 0.9
    torch.random.manual_seed(42)
    # da = (torch.randn_like(robot.l0) * 1e-2).requires_grad_()
    # da = torch.tensor([0.1]).requires_grad_()
    a = torch.rand_like(x_target).requires_grad_(True)
    ...
    for epoch in range(num_epochs):
        ...
        loss = (x_target - x).pow(2).sum() #+ torch.norm(x[0])**2
        print("loss\n", loss, "\nx\n", x[1])
        loss.backward()
        optimizer.step()
        # scheduler.step()
